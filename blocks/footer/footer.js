...
const SOCIAL_ICONS = {
  ...
  twitter:
    '<svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor" aria-hidden="true"><path d="M18.9 2H22l-7.3 8.3L23.3 22h-6.8l-5.3-6.9L5.1 22H2l7.8-8.9L1 2h7l4.8 6.3L18.9 2Zm-1.2 18h1.9L7.1 4H5.1l12.6 16Z"/></svg>',
...
export default async function decorate(block) {
  ...
  if (newsletter) {
    const subscribe = [...newsletter.querySelectorAll('a')].find((a) => /subscribe/i.test(a.textContent));
    if (subscribe) {
  ...
  const socialP = [...footer.querySelectorAll('p')].find((p) => p.querySelector(
    'a[href*="youtube"], a[href*="twitter"], a[href*="linkedin"]',
  ));
  if (socialP) {
  ...
  // Awards & Recognition: production #footer-logos (figure + linked logo images).
  const awardList = [...footer.querySelectorAll('ul')].find((ul) => ul.querySelector(
    'a[href*="softwareadvice"], a[href*="capterra"], a[href*="g2.com"]',
  ));
  if (awardList) {
